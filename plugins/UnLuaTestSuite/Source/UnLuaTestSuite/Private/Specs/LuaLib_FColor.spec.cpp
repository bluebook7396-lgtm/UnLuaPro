...
BEGIN_DEFINE_SPEC(FUnLuaLibFColorSpec, "UnLua.API.FColor", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::ServerContext)
    lua_State* L;
...
void FUnLuaLibFColorSpec::Define()
{
    ...
    Describe(TEXT("tostring()"), [this]
    {
        It(TEXT("转为字符串"), EAsyncExecution::TaskGraphMainThread, [this]()
        {
            ...
            TEST_EQUAL(FString(lua_tostring(L, -1)), TEXT("(R=1,G=2,B=3,A=4)"));
        });
    });
