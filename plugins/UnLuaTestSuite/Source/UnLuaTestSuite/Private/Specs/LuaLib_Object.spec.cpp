...
BEGIN_DEFINE_SPEC(FUnLuaLibObjectSpec, "UnLua.API.UObject", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::ServerContext)
    lua_State* L;
    UWorld* World;
...
void FUnLuaLibObjectSpec::Define()
{
    ...
    Describe(TEXT("GetName"), [this]()
    {
        It(TEXT("获取对象的名称"), EAsyncExecution::TaskGraphMainThread, [this]()
        {
            ...
            UnLua::RunChunk(L, Chunk);
            TEST_EQUAL(FString(lua_tostring(L, -1)), TEXT("Test"));
        });
    });
