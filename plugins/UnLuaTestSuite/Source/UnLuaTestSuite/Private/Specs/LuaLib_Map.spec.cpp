...
BEGIN_DEFINE_SPEC(FUnLuaLibMapSpec, "UnLua.API.TMap", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::ServerContext)
...
void FUnLuaLibMapSpec::Define()
{
    ...
    Describe(TEXT("构造TMap"), [this]
    {
        ...
        It(TEXT("构造TMap<FString,FString>"), EAsyncExecution::TaskGraphMainThread, [this]
        {
            ...
            TEST_EQUAL(Map->Num(), 2);
            TEST_EQUAL(Map->operator[]("A"), TEXT("Apple"));
            TEST_EQUAL(Map->operator[]("B"), TEXT("Banana"));
        });
